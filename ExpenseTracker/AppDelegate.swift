...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {
      ...
      guard let tabBarController = window?.rootViewController as? TabBarController,
        let vc = tabBarController.viewControllers?.first as? MonthExpenseVC,
            let store = vc.getExpenseStore() else {
            return
        }
    
       let success  = store.saveChanges()
        if (success) {
            print("Saved all of the expenses")
        } else {
            print("Could not save any of the expenses")
        }
    }
